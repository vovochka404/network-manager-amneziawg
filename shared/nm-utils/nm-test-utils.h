...
#define nmtst_assert_error(error, expect_error_domain, expect_error_code, expect_error_pattern)               \
    ...
    {                                                                                                         \
        ...
        if (_expect_error_pattern && !g_pattern_match_simple(_expect_error_pattern, _error->message)) {       \
    ...
    if (__nmtst_internal.orig_argv) {
        ...
        for (; *a; a++) {
            if (!g_ascii_strcasecmp(*a, "--debug"))
                ...
                is_debug = FALSE;
            else if (!strcmp(*a, "-m=slow") || !strcmp(*a, "-m=thorough") || !strcmp(*a, "-m=quick") || (!strcmp(*a, "-m") && *(a + 1) && (!strcmp(*(a + 1), "quick") || !strcmp(*(a + 1), "slow") || !strcmp(*(a + 1), "thorough"))))
...
#define NMTST_VARIANT_DROP_PROPERTY(__setting_name, __property_name)                                          \
    ...
    {                                                                                                         \
        if (__phase == NMTST_VARIANT_EDITOR_PROPERTY && __cur_property_name) {                                \
            if (!strcmp(__cur_setting_name, __setting_name) && !strcmp(__cur_property_name, __property_name)) \
                __cur_property_name = NULL;                                                                   \
        }                                                                                                     \
    }                                                                                                         \
    G_STMT_END
