...
{
    return GLIB_CHECK_VERSION(major, minor, micro) || ((glib_major_version > major) || (glib_major_version == major && glib_minor_version > minor) || (glib_major_version == major && glib_minor_version == minor && glib_micro_version < micro));
}
...
#define g_ptr_array_insert(array, index, data)                   \
    G_STMT_START { _nm_g_ptr_array_insert(array, index, data); } \
    G_STMT_END
#else
#define g_ptr_array_insert(array, index, data)  \
    G_STMT_START                                \
...
#define g_credentials_get_unix_pid(creds, error)                     \
    ({                                                               \
        G_GNUC_BEGIN_IGNORE_DEPRECATIONS(g_credentials_get_unix_pid) \
        ((creds), (error));                                          \
        G_GNUC_END_IGNORE_DEPRECATIONS                               \
    })
#else
