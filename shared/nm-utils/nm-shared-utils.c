                       ...
                       gboolean skip_repeated)
{
    ...
    j = 0;
    for (i = 0; strv[i]; i++) {
        if ((skip_empty && !*strv[i]) || (skip_repeated && nm_utils_strv_find_first(strv, j, strv[i]) >= 0))
            g_free(strv[i]);
        else
            strv[j++] = strv[i];
    }
                            ...
                            gboolean consider_is_disposing)
{
    if (error) {
        if (g_error_matches(error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
            return TRUE;
        if (consider_is_disposing && g_error_matches(error, NM_UTILS_ERROR, NM_UTILS_ERROR_CANCELLED_DISPOSING))
            return TRUE;
    }
    return FALSE;
}
...
nm_utils_str_utf8safe_escape(const char *str, NMUtilsStrUtf8SafeFlags flags, char **to_free)
{
    ...
    *to_free = NULL;
    ...
    if (g_utf8_validate(str, -1, &p) && !NM_STRCHAR_ANY(str, ch,
                                                        (ch == '\\' || (NM_FLAGS_HAS(flags, NM_UTILS_STR_UTF8_SAFE_FLAG_ESCAPE_CTRL) && ch < ' ') || (NM_FLAGS_HAS(flags, NM_UTILS_STR_UTF8_SAFE_FLAG_ESCAPE_NON_ASCII) && ((guchar)ch) >= 127))))
        return str;
    ...
    do {
        for (; str < p; str++) {
            ...
            if (ch == '\\')
                g_string_append(s, "\\\\");
            else if ((NM_FLAGS_HAS(flags, NM_UTILS_STR_UTF8_SAFE_FLAG_ESCAPE_CTRL) && ch < ' ') || (NM_FLAGS_HAS(flags, NM_UTILS_STR_UTF8_SAFE_FLAG_ESCAPE_NON_ASCII) && ((guchar)ch) >= 127))
