...
/* amneziawg.h netlink uapi: */

#define AWG_GENL_NAME "amneziawg"
#define AWG_GENL_VERSION 1
...
enum wgdevice_flag {
	AWGDEVICE_F_REPLACE_PEERS = 1U << 0
};
enum wgdevice_attribute {
	AWGDEVICE_A_UNSPEC,
	AWGDEVICE_A_IFINDEX,
	AWGDEVICE_A_IFNAME,
	AWGDEVICE_A_PRIVATE_KEY,
	AWGDEVICE_A_PUBLIC_KEY,
	AWGDEVICE_A_FLAGS,
	AWGDEVICE_A_LISTEN_PORT,
	AWGDEVICE_A_FWMARK,
	AWGDEVICE_A_PEERS,
	__AWGDEVICE_A_LAST
};
...
enum wgpeer_flag {
	AWGPEER_F_REMOVE_ME = 1U << 0,
	AWGPEER_F_REPLACE_ALLOWEDIPS = 1U << 1
};
enum wgpeer_attribute {
	AWGPEER_A_UNSPEC,
	AWGPEER_A_PUBLIC_KEY,
	AWGPEER_A_PRESHARED_KEY,
	AWGPEER_A_FLAGS,
	AWGPEER_A_ENDPOINT,
	AWGPEER_A_PERSISTENT_KEEPALIVE_INTERVAL,
	AWGPEER_A_LAST_HANDSHAKE_TIME,
	AWGPEER_A_RX_BYTES,
	AWGPEER_A_TX_BYTES,
	AWGPEER_A_ALLOWEDIPS,
	__AWGPEER_A_LAST
};

enum wgallowedip_attribute {
	AWGALLOWEDIP_A_UNSPEC,
	AWGALLOWEDIP_A_FAMILY,
	AWGALLOWEDIP_A_IPADDR,
	AWGALLOWEDIP_A_CIDR_MASK,
	__AWGALLOWEDIP_A_LAST
};
...
static int parse_linkinfo(const struct nlattr *attr, void *data)
{
	...
	if (mnl_attr_get_type(attr) == IFLA_INFO_KIND && !strcmp(AWG_GENL_NAME, mnl_attr_get_str(attr)))
		buffer->good = true;
	return MNL_CB_OK;
}
...
static int add_del_iface(const char *ifname, bool add)
{
	...
	mnl_attr_put_strz(nlh, IFLA_INFO_KIND, AWG_GENL_NAME);
...
int awg_set_device(awg_device *dev)
{
	...
	nlg = mnlg_socket_open(AWG_GENL_NAME, AWG_GENL_VERSION);
	if (!nlg)
		return -errno;

again:
	nlh = mnlg_msg_prepare(nlg, AWG_CMD_SET_DEVICE, NLM_F_REQUEST | NLM_F_ACK);
	mnl_attr_put_strz(nlh, AWGDEVICE_A_IFNAME, dev->name);

	if (!peer) {
		uint32_t flags = 0;

		if (dev->flags & AWGDEVICE_HAS_PRIVATE_KEY)
			mnl_attr_put(nlh, AWGDEVICE_A_PRIVATE_KEY, sizeof(dev->private_key), dev->private_key);
		if (dev->flags & AWGDEVICE_HAS_LISTEN_PORT)
			mnl_attr_put_u16(nlh, AWGDEVICE_A_LISTEN_PORT, dev->listen_port);
		if (dev->flags & AWGDEVICE_HAS_FWMARK)
			mnl_attr_put_u32(nlh, AWGDEVICE_A_FWMARK, dev->fwmark);
		if (dev->flags & AWGDEVICE_REPLACE_PEERS)
			flags |= AWGDEVICE_F_REPLACE_PEERS;
		if (flags)
			mnl_attr_put_u32(nlh, AWGDEVICE_A_FLAGS, flags);
	}
	if (!dev->first_peer)
		goto send;
	peers_nest = peer_nest = allowedips_nest = allowedip_nest = NULL;
	peers_nest = mnl_attr_nest_start(nlh, AWGDEVICE_A_PEERS);
	for (i = 0, peer = peer ? peer : dev->first_peer; peer; peer = peer->next_peer) {
		...
		if (!peer_nest)
			goto toobig_peers;
		if (!mnl_attr_put_check(nlh, MNL_SOCKET_BUFFER_SIZE, AWGPEER_A_PUBLIC_KEY, sizeof(peer->public_key), peer->public_key))
			goto toobig_peers;
		if (peer->flags & AWGPEER_REMOVE_ME)
			flags |= AWGPEER_F_REMOVE_ME;
		if (!allowedip) {
			if (peer->flags & AWGPEER_REPLACE_ALLOWEDIPS)
				flags |= AWGPEER_F_REPLACE_ALLOWEDIPS;
			if (peer->flags & AWGPEER_HAS_PRESHARED_KEY) {
				if (!mnl_attr_put_check(nlh, MNL_SOCKET_BUFFER_SIZE, AWGPEER_A_PRESHARED_KEY, sizeof(peer->preshared_key), peer->preshared_key))
					goto toobig_peers;
			}
			if (peer->endpoint.addr.sa_family == AF_INET) {
				if (!mnl_attr_put_check(nlh, MNL_SOCKET_BUFFER_SIZE, AWGPEER_A_ENDPOINT, sizeof(peer->endpoint.addr4), &peer->endpoint.addr4))
					goto toobig_peers;
			} else if (peer->endpoint.addr.sa_family == AF_INET6) {
				if (!mnl_attr_put_check(nlh, MNL_SOCKET_BUFFER_SIZE, AWGPEER_A_ENDPOINT, sizeof(peer->endpoint.addr6), &peer->endpoint.addr6))
					goto toobig_peers;
			}
			if (peer->flags & AWGPEER_HAS_PERSISTENT_KEEPALIVE_INTERVAL) {
				if (!mnl_attr_put_u16_check(nlh, MNL_SOCKET_BUFFER_SIZE, AWGPEER_A_PERSISTENT_KEEPALIVE_INTERVAL, peer->persistent_keepalive_interval))
					goto toobig_peers;
			}
		}
		if (flags) {
			if (!mnl_attr_put_u32_check(nlh, MNL_SOCKET_BUFFER_SIZE, AWGPEER_A_FLAGS, flags))
				goto toobig_peers;
		}
		if (peer->first_allowedip) {
			if (!allowedip)
				allowedip = peer->first_allowedip;
			allowedips_nest = mnl_attr_nest_start_check(nlh, MNL_SOCKET_BUFFER_SIZE, AWGPEER_A_ALLOWEDIPS);
			if (!allowedips_nest)
			...
			for (j = 0; allowedip; allowedip = allowedip->next_allowedip) {
				...
				if (!allowedip_nest)
					goto toobig_allowedips;
				if (!mnl_attr_put_u16_check(nlh, MNL_SOCKET_BUFFER_SIZE, AWGALLOWEDIP_A_FAMILY, allowedip->family))
					goto toobig_allowedips;
				if (allowedip->family == AF_INET) {
					if (!mnl_attr_put_check(nlh, MNL_SOCKET_BUFFER_SIZE, AWGALLOWEDIP_A_IPADDR, sizeof(allowedip->ip4), &allowedip->ip4))
						goto toobig_allowedips;
				} else if (allowedip->family == AF_INET6) {
					if (!mnl_attr_put_check(nlh, MNL_SOCKET_BUFFER_SIZE, AWGALLOWEDIP_A_IPADDR, sizeof(allowedip->ip6), &allowedip->ip6))
						goto toobig_allowedips;
				}
				if (!mnl_attr_put_u8_check(nlh, MNL_SOCKET_BUFFER_SIZE, AWGALLOWEDIP_A_CIDR_MASK, allowedip->cidr))
					goto toobig_allowedips;
...
static int parse_allowedip(const struct nlattr *attr, void *data)
{
	...
	case AWGALLOWEDIP_A_UNSPEC:
		break;
	case AWGALLOWEDIP_A_FAMILY:
		if (!mnl_attr_validate(attr, MNL_TYPE_U16))
			allowedip->family = mnl_attr_get_u16(attr);
		break;
	case AWGALLOWEDIP_A_IPADDR:
...
static int parse_peer(const struct nlattr *attr, void *data)
{
	...
	switch (mnl_attr_get_type(attr)) {
	case AWGPEER_A_UNSPEC:
		break;
	case AWGPEER_A_PUBLIC_KEY:
		if (mnl_attr_get_payload_len(attr) == sizeof(peer->public_key)) {
			...
			peer->flags |= AWGPEER_HAS_PUBLIC_KEY;
		}
		break;
	case AWGPEER_A_PRESHARED_KEY:
		if (mnl_attr_get_payload_len(attr) == sizeof(peer->preshared_key)) {
			...
			if (!awg_key_is_zero(peer->preshared_key))
				peer->flags |= AWGPEER_HAS_PRESHARED_KEY;
		}
		break;
	case AWGPEER_A_ENDPOINT: {
	...
	case AWGPEER_A_PERSISTENT_KEEPALIVE_INTERVAL:
		if (!mnl_attr_validate(attr, MNL_TYPE_U16))
			peer->persistent_keepalive_interval = mnl_attr_get_u16(attr);
		break;
	case AWGPEER_A_LAST_HANDSHAKE_TIME:
		...
		break;
	case AWGPEER_A_RX_BYTES:
		if (!mnl_attr_validate(attr, MNL_TYPE_U64))
			peer->rx_bytes = mnl_attr_get_u64(attr);
		break;
	case AWGPEER_A_TX_BYTES:
		if (!mnl_attr_validate(attr, MNL_TYPE_U64))
			peer->tx_bytes = mnl_attr_get_u64(attr);
		break;
	case AWGPEER_A_ALLOWEDIPS:
...
{
	awg_device *device = data;
	awg_peer *new_peer = calloc(1, sizeof(awg_peer));
	int ret;
	...
	if (!ret)
		return ret;
	if (!(new_peer->flags & AWGPEER_HAS_PUBLIC_KEY)) {
		errno = ENXIO;
		return MNL_CB_ERROR;
	}
	return MNL_CB_OK;
...
static int parse_device(const struct nlattr *attr, void *data)
{
	...
	switch (mnl_attr_get_type(attr)) {
	case AWGDEVICE_A_UNSPEC:
		break;
	case AWGDEVICE_A_IFINDEX:
		if (!mnl_attr_validate(attr, MNL_TYPE_U32))
			device->ifindex = mnl_attr_get_u32(attr);
		break;
	case AWGDEVICE_A_IFNAME:
		if (!mnl_attr_validate(attr, MNL_TYPE_STRING)) {
			strncpy(device->name, mnl_attr_get_str(attr), sizeof(device->name) - 1);
			device->name[sizeof(device->name) - 1] = '\0';
		}
		break;
	case AWGDEVICE_A_PRIVATE_KEY:
		if (mnl_attr_get_payload_len(attr) == sizeof(device->private_key)) {
			...
			device->flags |= AWGDEVICE_HAS_PRIVATE_KEY;
		}
		break;
	case AWGDEVICE_A_PUBLIC_KEY:
		if (mnl_attr_get_payload_len(attr) == sizeof(device->public_key)) {
			...
			device->flags |= AWGDEVICE_HAS_PUBLIC_KEY;
		}
		break;
	case AWGDEVICE_A_LISTEN_PORT:
		if (!mnl_attr_validate(attr, MNL_TYPE_U16))
			device->listen_port = mnl_attr_get_u16(attr);
		break;
	case AWGDEVICE_A_FWMARK:
		if (!mnl_attr_validate(attr, MNL_TYPE_U32))
			device->fwmark = mnl_attr_get_u32(attr);
		break;
	case AWGDEVICE_A_PEERS:
...
static void coalesce_peers(awg_device *device)
{
	awg_peer *old_next_peer, *peer = device->first_peer;
...
int awg_get_device(awg_device **device, const char *device_name)
{
	int ret = 0;
	struct nlmsghdr *nlh;
	struct mnlg_socket *nlg;
...
try_again:
	*device = calloc(1, sizeof(awg_device));
	if (!*device)
		return -errno;

	nlg = mnlg_socket_open(AWG_GENL_NAME, AWG_GENL_VERSION);
	if (!nlg) {
		awg_free_device(*device);
		*device = NULL;
		return -errno;
	}

	nlh = mnlg_msg_prepare(nlg, AWG_CMD_GET_DEVICE, NLM_F_REQUEST | NLM_F_ACK | NLM_F_DUMP);
	mnl_attr_put_strz(nlh, AWGDEVICE_A_IFNAME, device_name);
	if (mnlg_socket_send(nlg, nlh) < 0) {
		ret = -errno;
		goto out;
	}
...
char *awg_list_device_names(void)
{
...
{
	awg_generate_preshared_key(private_key);
